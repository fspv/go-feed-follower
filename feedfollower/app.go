...
// A single instance of a feed update, which is generated by the update
// generator and later passed all the way to the notification channel
...
func (feedUpdate *FeedUpdate) Hash() string {
	// Should be hashable to avoid duplication We use precalculated value, but
...
func (notificationChannel *TelegramNotificationChannel) Run() {
	/*
		Initialize the telegram notification channel and listen for the feed
		updates indefinitely. Once the update is received, check if it has been
		sent already in the database and if it wasn't, send a new telegram message
	*/
...
// Observer and observable concrete implementation for the user processor. It
// will listen for the feed updates and once received, will notify all the
...
func (userProcessor *UserProcessor) SyncNotificationChannels() {
	/*
		Check if there are new notification channels for this user and
